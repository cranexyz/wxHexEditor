...
void HexEditor::OnFileModify(wxFileSystemWatcherEvent &event) {
	...
	if( event.GetChangeType()&wxFSW_EVENT_MODIFY ) {
		//if( event.GetPath().GetFullPath() == myfile->GetFileName().GetFullPath())
		Reload();
		}
	event.Skip(true);
	}
...
bool HexEditor::DeleteSelection( void ) {
	...
	bool success=false;
	if( select->GetState() ) {
		success = myfile->Add( std::min(select->StartOffset, select->EndOffset), NULL, -(int64_t)select->GetSize(), true );
		if(success)
			MoveTAGs( std::min(select->StartOffset, select->EndOffset), -(int64_t)select->GetSize() );
